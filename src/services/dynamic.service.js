...
const handleElasticIndexSchema = async (indexName) => {
  const _index = `${project.codename}_${indexName}`;
  const indexExists = await checkIndexExists(_index);
  if (!indexExists) {
    throw new ApiError(httpStatus.NOT_FOUND, "Index not found");
  }
  ...
  const filtirableFields = fieldBuilder(mapping[indexName].mappings.properties);

  const sortableFields = fieldBuilder(mapping[indexName].mappings.properties);
...
const handleGetFiltersElasticIndex = async (
  ...
  const body = await search(
    "filter_indexs",
    {
      query: {
        bool: {
          must: [{ match: { userId: userId } }, { match: { key: indexName } }],
        },
      },
    },
    page,
